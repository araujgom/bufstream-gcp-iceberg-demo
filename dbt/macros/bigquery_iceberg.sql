{% macro bigquery__create_table_as(temporary, relation, compiled_code, language='sql') -%}
  {%- if language == 'sql' -%}
    ...
    {%- if table_format == "iceberg" and partition_config is not none-%}
        {% do exceptions.raise_compiler_error("Partition by not available in iceberg tables, use cluster by instead") %}
    ...
    {% if table_format == "iceberg" %}
        {% set storage_uri = config.get('storage_uri', none) %}
        {% if storage_uri is not none %}
            {% set base_location = storage_uri %}
        {% else %}
        ...
        {%- endif %}
        {% set storage_sub_path = config.get('storage_sub_path', none) %}
        {% if storage_sub_path is not none %}
            {% set sub_path = storage_sub_path %}
        {% else %}
            {% set sub_path = relation.identifier %}
        {% endif %}

        {# Build connection path from env variables #}
        {% set bq_connection_id = env_var('BQ_CONNECTION_ID') %}
        {% set gcp_project_id = env_var('GCP_PROJECT_ID') %}
        {% set bq_location = env_var('BQ_LOCATION') %}
        {% set connection_path = "projects/" ~ gcp_project_id ~ "/locations/" ~ bq_location ~ "/connections/" ~ bq_connection_id %}
